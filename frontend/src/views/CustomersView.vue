...
async function onImportFileChange(e) {
  ...
  if (!/\.xlsx$/i.test(file.name)) {
    importError.value = 'Hanya file .xlsx yang didukung'
    return
  }
...
async function doImport() {
  ...
  try {
    ...
    if (e.response?.data?.detail) importDetail.value = e.response.data.detail
  } finally {
    importing.value = false
  }
}

// Import dijalankan asinkron di server: pantau statusnya sampai selesai.
async function pollImport(jobId) {
  for (;;) {
    await sleep(1500)
    let data
    try {
      const res = await api.get(`/customers/import/${jobId}`)
      data = res.data
    } catch (e) {
      importError.value = e.response?.data?.error || 'Gagal membaca status impor'
      if (e.response?.data?.detail) importDetail.value = e.response.data.detail
      return
    }
    ...
    if (data.error) {
      ...
      importResult.value = `Berhasil: ${data.imported || 0} diimpor`
      if (data.updated) importResult.value += `, ${data.updated} diperbarui`
      if (data.skipped) importResult.value += `, ${data.skipped} dilewati`
    }
...
async function exportExcel() {
  try {
    ...
    importError.value = e.response?.data?.error || 'Gagal mengekspor file'
  }
}
...
async function downloadTemplate() {
  try {
    ...
    importError.value = e.response?.data?.error || 'Gagal mengunduh template'
  }
}
...
<template>
  <div class="crud panel">
    <div class="crud__toolbar">
      ...
      <button v-if="auth.isAdmin" class="btn btn--ghost btn--sm" :disabled="importing" @click="pickImportFile">
        {{ importing ? 'Mengimpor...' : 'Import Excel' }}
      </button>
      <button class="btn btn--ghost btn--sm" @click="downloadTemplate">Template</button>
      <button class="btn btn--ghost btn--sm" @click="exportExcel">Export Excel</button>
      <button v-if="auth.isAdmin" class="btn btn--primary btn--sm" @click="openCreate">+ Tambah Pelanggan</button>
    </div>

    <input id="excel-import-input" type="file" accept=".xlsx" class="hidden-file" @change="onImportFileChange" />
