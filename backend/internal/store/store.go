...
func (s *Store) ApplyPingOutcome(ctx context.Context, a ping.PingAttempt) (*ping.PingOutcome, error) {
	...
	if newStatus == "ONLINE" {
		lastOnline = now
	} else if newStatus == "OFFLINE" && cu.Status != "OFFLINE" {
		// Simpan waktu mulai down hanya saat transisi ke OFFLINE, bukan pada
		// setiap ping gagal berikutnya. Ini dipakai frontend untuk menampilkan
		// durasi down pelanggan.
		lastOffline = now
	}
