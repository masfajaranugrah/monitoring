...
// diskUsageAll menjumlahkan kapasitas seluruh disk lokal (df -l) supaya
// dashboard menampilkan total disk server, bukan satu mount saja.
func diskUsageAll() (total, used uint64) {
	out, err := exec.Command("df", "-k", "-l").CombinedOutput()
	if err != nil {
		return 0, 0
	}
	lines := strings.Split(string(out), "\n")
	for _, line := range lines {
		...
		if len(fields) < 4 || !isAllDigits(fields[1]) {
			continue
		}
		tBlocks, err1 := strconv.ParseUint(fields[1], 10, 64)
		uBlocks, err2 := strconv.ParseUint(fields[2], 10, 64)
